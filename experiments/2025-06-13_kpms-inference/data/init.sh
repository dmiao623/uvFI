...
for src_dir in "${src_dirs[@]}"; do
    ...
    dest_dir="/projects/kumar-lab/miaod/projects/uvFI/experiments/2025-06-13_kpms-inference/data/videos"
    ...
    find "$src_dir" -type f -iname "*_trimmed_pose_est_v6.h5" | while read -r filepath; do
        ...
        date_folder=$(basename "$(dirname "$filepath")")
        subfolder=$(basename "$(dirname "$(dirname "$filepath")")")

        filename="${subfolder}__${date_folder}__${basename}"
        new_filename="${name}_${filename/_trimmed_pose_est_v6.h5/_pose_est_v6.h5}"
        if [ -L "$dest_dir/poses/$new_filename" ]; then
                 echo "transfer of ${filepath} failed; file ${new_filename} exists"
        else
